...
## retrospect との関係（学習ループ）

`ooda-loop:operate` はシングルループ（dispatch→observe→decide）で tier 割り当てを**実行する**側、`retrospect` はダブルループで割り当て方針を**較正する**側。両者は同じ4 tier 分類（Fable/Opus/Sonnet/Haiku）を共有し、`~/.claude/docs/effort-calibration.md`（playbook）を介して繋がる — operate は dispatch 時に condition→tier ヒューリスティックを読み、台帳のサイクル履歴に使用 tier を残す。retrospect はその履歴を証拠に tier 選択の過不足を判定し、playbook を更新する。運用上の接点は2つ: (1) ループ終結時（operate Step 5）に retrospect を発火させ、破棄前の台帳サイクル履歴を証拠にする。(2) 次ループの Orient（Step 4）が更新後の playbook を世襲遺産として引く。これで「実行→実績記録→較正→次の実行」の学習ループが1周閉じる。
...
- 台帳は固定1ファイル（`<scratchpad>/ooda-loop-ledger.md`）。進行中のループと無関係な新規ループが同時に発生した場合、後者の新規ループ作成が前者の台帳を上書きしうる（実装計画のTask 2最終レビューで発見、初版では対応を見送り）。複数ループの同時進行が実運用で実際に発生するかを見てから、台帳のファイル名にトピック識別子を含める等の対応を検討する。
