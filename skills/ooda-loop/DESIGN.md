...
`ooda-loop:operate` はシングルループ（dispatch→observe→decide）で tier 割り当てを**実行する**側、`retrospect` はダブルループで割り当て方針を**較正する**側。両者は同じ4 tier 分類（Fable/Opus/Sonnet/Haiku）を共有し、`~/.claude/docs/effort-calibration.md`（playbook）を介して繋がる — operate は dispatch 時に condition→tier ヒューリスティックを読み、台帳のサイクル履歴に使用 tier を残す。retrospect はその履歴を証拠に tier 選択の過不足を判定し、playbook を更新する。これで「実行→実績記録→較正→次の実行」の学習ループが閉じる。
