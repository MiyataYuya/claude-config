...
- **playbook（世襲遺産）に照らす**: `~/.claude/docs/effort-calibration.md` は過去ループの retrospect が積み上げた「前回までの経験」。今の観測が既知の condition→tier や既知の落とし穴に一致するか、過去に反証済みの筋を再びたどっていないかを引いて、解釈と次の Decide に効かせる（Boyd の Orient に heritage/experience を流し込む一手）
...
**終結時の retrospect（必須）**: 「ユーザーに報告」で解決した場合も「打ち切り」の場合も、台帳を「状態: 終結」に更新する**直前**に `retrospect` スキルを実施する。台帳のサイクル履歴（各 Act の使用 tier と合否）がそのまま tier 較正の証拠になる — 終結後は台帳が破棄/アーカイブされうるので順序が重要。retrospect が playbook を更新し、その playbook を次ループの Orient（Step 4）が世襲遺産として引く。これで学習ループが1周する。
