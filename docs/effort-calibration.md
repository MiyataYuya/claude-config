...
- **flaky なブラウザモードテスト（vitest browser 等）の結果を判断根拠にする時 → 合否はテスト名の出力行で判定、サマリ数値では判定しない**。ブラウザ死亡時は `1 passed (3)` が「残り未実行」を意味し、`tests 0ms` で内訳が消える *(2026-07-23 WI810/812: サマリ誤読で誤修正1件 revert、委譲 agent も stderr grep 判定で誤結論)*
